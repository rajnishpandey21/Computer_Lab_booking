(function () {
  ...
  const LOADING_COPY = {
    branchOptions: [
      "We have received your request.",
      "Loading branch list.",
      "Preparing the dashboard access."
    ],
    studentLogin: [
      "We have received your request.",
      "Checking registration number.",
      "Verifying your center details.",
      "Loading your slot dashboard."
    ],
    branchLogin: [
      "We have received your request.",
      "Checking branch passcode.",
      "Loading branch occupancy.",
      "Preparing today's dashboard."
    ],
    slots: [
      "We have received your request.",
      "Now checking availability.",
      "Refreshing slot occupancy.",
      "Bringing the latest slots."
    ],
    history: [
      "We have received your request.",
      "Loading your previous bookings.",
      "Sorting upcoming and past slots."
    ],
    branchDashboard: [
      "We have received your request.",
      "Checking today's branch occupancy.",
      "Preparing slot-wise booking data.",
      "Updating the branch dashboard."
    ],
    book: [
      "We have received your request.",
      "Now checking availability.",
      "Booking room for you.",
      "Generating ticket.",
      "Saving your booking."
    ],
    cancel: [
      ...
      "Releasing the computer instantly.",
      "Refreshing your updated bookings."
    ]
  };
  const BUTTON_LOADING_COPY = {
    studentLogin: [
      "Checking...",
      "Verifying Regno...",
      "Bas 2 Minute Lagega...",
  ...
  function cacheElements() {
    ...
    elements.dateRailDots = document.getElementById("dateRailDots");
    ...
    elements.adminDateRailDots = document.getElementById("adminDateRailDots");
    ...
    elements.loadingHudText = document.getElementById("loadingHudText");
  }
  ...
  async function loadBranchOptions(showLoading) {
    if (!isApiConfigured()) {
      renderBranchOptions();
      return;
    }
  ...
  async function loginBranchHead(branch, passcode) {
    ...
    setButtonLoading(elements.branchLoginButton, true, "Checking...", "branchLogin");
    beginLoading("branchLogin");
    ...
    try {
      ...
      await loadBranchDashboard(state.adminSelectedDate, { silent: true });
  ...
  async function loginWithRegno(regno) {
    ...
    setButtonLoading(elements.loginButton, true, "Checking...", "studentLogin");
    beginLoading("studentLogin");
    ...
    try {
      ...
      applyStudentAvailability(data);
      elements.regnoInput.value = regno;
      renderStudentProfile();
      hydrateDateRail();
      if (state.selectedDate) {
        await Promise.all([
          loadSlots(state.selectedDate, { silent: true }),
          loadHistory({ silent: true })
        ]);
      } else {
        renderNoAvailableDatesState();
      ...
      toast(getStudentLoginErrorMessage(error));
      logout(false);
    } finally {
      endLoading();
  ...
  function logout(showToast) {
    ...
    elements.dateRailDots.innerHTML = "";
    elements.adminDateRail.innerHTML = "";
    elements.adminDateRailDots.innerHTML = "";
  ...
  function hydrateDateRail() {
    const dates = Array.isArray(state.availableDates) ? state.availableDates.slice() : [];
    ...
    if (!dates.length) {
      ...
      elements.dateRail.innerHTML = "";
      elements.dateRailDots.innerHTML = "";
      elements.selectedDateBadge.textContent = "No dates";
      return;
    }
    ...
    elements.dateRail.innerHTML = dates.map(function (dateKey, index) {
      ...
      const helper = getDateChipHelperLabel(dateKey, todayKey, formatted.shortDay);
    ...
    renderDateDots(elements.dateRailDots, dates, state.selectedDate, function (dateKey) {
      state.selectedDate = dateKey;
      hydrateDateRail();
      loadSlots(state.selectedDate);
    });

    elements.selectedDateBadge.textContent = state.selectedDate ? formatDateLabel(state.selectedDate).longDate : "No dates";
  ...
  function hydrateAdminDateRail() {
    ...
    renderDateDots(elements.adminDateRailDots, dates, state.adminSelectedDate, function (dateKey) {
      state.adminSelectedDate = dateKey;
      hydrateAdminDateRail();
      loadBranchDashboard(state.adminSelectedDate);
    });
  ...
  async function loadBranchDashboard(dateKey, options) {
    if (!state.branchSession) {
      return;
    }

    const shouldShowLoading = !options || options.silent !== true;
  ...
  async function loadSlots(dateKey, options) {
    if (!state.student) {
      return;
    }
    ...
    try {
      ...
      if (state.availableDates.indexOf(dateKey) === -1) {
        state.selectedDate = state.availableDates[0];
        hydrateDateRail();
        await loadSlots(state.selectedDate, { silent: true });
        return;
      }
  ...
  function applyStudentAvailability(payload) {
    state.availableDates = Array.isArray(payload && payload.availableDates) ? payload.availableDates.slice() : [];
    state.availabilityMessage = String((payload && payload.availabilityMessage) || "").trim();
  }

  function renderNoAvailableDatesState() {
    elements.slotMessage.textContent = state.availabilityMessage || "No booking dates are available for your course in the next 7 days.";
    elements.slotMessage.classList.remove("is-hidden");
    elements.slotGrid.innerHTML = '<div class="empty-state">No booking days are available right now.</div>';
  }
  ...
  async function confirmBooking() {
    ...
    setButtonLoading(elements.confirmBookingButton, true, "Booking...", "book");
    beginLoading("book");
  ...
  async function cancelBooking(bookingId, triggerButton) {
    if (!window.confirm("Cancel this booking? This will immediately free the slot.")) {
      return;
    }

    if (triggerButton) {
      setButtonLoading(triggerButton, true, "Cancelling...", "cancel");
    }
    beginLoading("cancel");
    ...
    try {
      ...
      endLoading();
      if (triggerButton) {
        setButtonLoading(triggerButton, false, "Cancel Booking");
      }
    }
  }
  ...
  function renderDateDots(container, dates, activeDate, onSelect) {
    if (!container) {
      return;
    }

    container.innerHTML = dates.map(function (dateKey) {
      const activeClass = dateKey === activeDate ? " is-active" : "";
      return (
        '<button class="date-dot' + activeClass + '" type="button" aria-label="' +
        escapeHtml(formatDateLabel(dateKey).longDate) +
        '"></button>'
      );
    }).join("");

    Array.from(container.querySelectorAll(".date-dot")).forEach(function (button, index) {
      button.addEventListener("click", function () {
  ...
  function getStudentLoginErrorMessage(error) {
    ...
    if (/not found|invalid|unknown|incorrect/i.test(message)) {
      return "Your Registration Number is Wrong, Please Enter Correct regno";
    }

    return message || "Your Registration Number is Wrong, Please Enter Correct regno";
  }

  function beginLoading(copyKey) {
    stopLoading();
  }
  ...
  function stopLoading() {
    if (state.loadingTimerId) {
      window.clearInterval(state.loadingTimerId);
    ...
    if (elements.loadingHud) {
      elements.loadingHud.classList.add("is-hidden");
    }
    if (elements.loadingHudText) {
      elements.loadingHudText.textContent = DEFAULT_LOADING_MESSAGES[0];
    }
  }

  function setLoadingMessages(copyKey) {
    void copyKey;
    ...
    const messages = Array.isArray(copyKey) ? copyKey : (LOADING_COPY[copyKey] || DEFAULT_LOADING_MESSAGES);
    ...
    state.loadingMessages = messages.slice();
    state.loadingIndex = 0;
    elements.loadingHudText.textContent = state.loadingMessages[0] || DEFAULT_LOADING_MESSAGES[0];

    if (state.loadingMessages.length < 2) {
      return;
    }

    state.loadingTimerId = window.setInterval(function () {
      if (state.loadingIndex >= state.loadingMessages.length - 1) {
        window.clearInterval(state.loadingTimerId);
        state.loadingTimerId = 0;
        return;
      }
      ...
      elements.loadingHudText.textContent = state.loadingMessages[state.loadingIndex];
    }, 980);
  }
  ...
  function setButtonLoading(button, isLoading, label, copyKey) {
    if (!button) {
      return;
    }
    ...
    stopButtonLoadingSequence(button);
    button.disabled = isLoading;
    button.classList.toggle("is-loading", isLoading);

    if (isLoading) {
      startButtonLoadingSequence(button, label, copyKey);
      return;
    }

    button.textContent = label || button.dataset.defaultLabel || "Please wait";
  }

  function startButtonLoadingSequence(button, label, copyKey) {
    const messages = getButtonLoadingMessages(label, copyKey);

    updateButtonLoadingMarkup(button, messages[0] || label || "Please wait");

    if (messages.length < 2) {
      return;
    }

    button._loadingSequence = {
      index: 0,
      messages: messages.slice(),
      timerId: window.setInterval(function () {
        const sequence = button._loadingSequence;
        if (!sequence) {
          return;
        }

        if (sequence.index >= sequence.messages.length - 1) {
          window.clearInterval(sequence.timerId);
          sequence.timerId = 0;
          return;
        }

        sequence.index += 1;
        updateButtonLoadingMarkup(button, sequence.messages[sequence.index]);
      }, 980)
    };
  }

  function stopButtonLoadingSequence(button) {
    if (!button || !button._loadingSequence) {
      return;
    }

    if (button._loadingSequence.timerId) {
      window.clearInterval(button._loadingSequence.timerId);
    }

    button._loadingSequence = null;
  }

  function updateButtonLoadingMarkup(button, text) {
    button.innerHTML = '<span class="button-spinner" aria-hidden="true"></span><span class="button-label"></span>';
    button.querySelector(".button-label").textContent = text;
  }

  function getButtonLoadingMessages(label, copyKey) {
    const messages = Array.isArray(copyKey) ? copyKey : (BUTTON_LOADING_COPY[copyKey] || []);
    if (messages.length) {
  ...
  function getDateChipHelperLabel(dateKey, todayKey, fallbackLabel) {
    const normalizedTodayKey = todayKey || getIstTodayKey();
    if (dateKey === normalizedTodayKey) {
      return "Today";
    }

    if (dateKey === addDaysToDateKey(normalizedTodayKey, 1)) {
      return "Tomorrow";
    }

    return fallbackLabel || "Available";
  }
